...
export default function TipsCard({ title, bold, notBold, animated, link }) {
  ...
  async function handleEnterAnimate() {
    animate(divRef.current, { backgroundColor: '#ffffffff' }, { duration: 0.25 });
  ...
  async function handleLeaveAnimate() {
    animate(divRef.current, { backgroundColor: '#ffffff00' }, { duration: 0.25 });
  ...
  return (
    ...
    >
      <div 
        className="absolute w-4/5 h-3/5 left-[10%] top-[20%] flex flex-col justify-between font-montserrat text-[#F05814] sm:text-xl font-bold text-3xl md:text-3xl lg:text-4xl"
      >
        <h1
          className=""
        >
        ...
        <p
          ...
          ref={textRef1}
        >
          <span className="font-bold">{bold} </span>{notBold}
        </p>
        {animated && <p className="text-black font-light sm:text-xs absolute top-1/2 text-base md:text-sm lg:text-lg" ref={textRef2} style={{ opacity: 0 }}>
          {animated}
        </p>}
        {link && (
          <Link
